...
const CATEGORY_ID = 1; // Vêtements Homme
const WHATSAPP_NUMBER = "22893733150";

const VetementsHomme = () => {
  ...
  const filters = ["Tout", "Hoodies", "T-shirts", "Pantalons", "Vestes"];
  ...
  const filterArticles = (filter) => {
    if (filter === "Tout") {
      ...
      const filtered = articles.filter((article) => {
        ...
        switch (filter) {
          case "Hoodies":
            return searchText.includes("hoodie") || searchText.includes("sweat") || searchText.includes("capuche");
          case "T-shirts":
            return searchText.includes("t-shirt") || searchText.includes("tee") || searchText.includes("shirt");
          case "Pantalons":
            return searchText.includes("pantalon") || searchText.includes("jean") || searchText.includes("jogger") || searchText.includes("cargo");
          case "Vestes":
            return searchText.includes("veste") || searchText.includes("jacket") || searchText.includes("bomber") || searchText.includes("blouson");
          default:
            return true;
        }
      });
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-black text-white mb-6">
            STREETWEAR <span className="text-gradient">HOMME</span>
          </h1>
          <p className="text-xl text-gray-400 max-w-3xl mx-auto">
            Hoodies, t-shirts, joggers et plus. Le style urbain qui te représente.
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredArticles.map((article) => (
              <div
                key={article.id}
                className="group bg-gray-900 rounded-2xl overflow-hidden border border-gray-800 hover:border-orange-500/50 transition-all duration-300 card-glow"
              >
                {/* Image */}
                ...
                <div className="p-5">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      <p className="text-orange-400 text-sm font-semibold">{article.marque}</p>
        ...
        {filteredArticles.length > 0 && (
          <div className="text-center mt-12">
            <p className="text-gray-500 text-sm mb-4">{filteredArticles.length} article(s) affiché(s)</p>
          </div>
        )}
      </div>
    </div>
